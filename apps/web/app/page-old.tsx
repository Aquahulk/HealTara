...
export default function HomePage() {
  ...
  const defaultHomepageData: HomepageData = {
    hero: {
      title: "Find and Book at Top Hospitals",
      subtitle: "Discover trusted hospitals and departments. Book appointments fast with a modern, hospital-first experience.",
      searchPlaceholder: "Search hospitals, departments, or location...",
      ctaText: "Find a Hospital Now",
      backgroundImage: "linear-gradient(135deg, #0061ff 0%, #60efff 100%)"
    },
    stats: {
      doctors: 1250,
      patients: 50000,
    ...
    features: [
      ...
      },
      {
        ...
        color: "linear-gradient(135deg, #00ff87 0%, #60efff 100%)"
      },
      {
        icon: "🌐",
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [hospitalsRes, homepageRes] = await Promise.allSettled([
  ...
  // Load doctors when sort changes (reset pagination)
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        setLoading(true);
        const list = await apiClient.getDoctors({ sort: sortBy, page: 1, pageSize });
        setDoctors(list || []);
        setPage(1);
  ...
  const loadMore = async () => {
    try {
      ...
      const list = await apiClient.getDoctors({ sort: sortBy, page: nextPage, pageSize });
      const newItems = list || [];
      setDoctors(prev => [...prev, ...newItems]);
      setPage(nextPage);
      setHasMore(newItems.length === pageSize);
    } catch (error) {
      console.warn('Failed to load more doctors:', error);
      setHasMore(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-16">
      ...
      {/* ============================================================================ */}
      <section
        className="relative py-8 md:py-10 px-4 text-white"
      ...
      >
        ...
        <div className="relative max-w-6xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg mb-5 max-w-2xl mx-auto opacity-90">
            {data.hero.subtitle}
          </p>
          ...
          <div className="max-w-xl mx-auto mb-5">
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-2.5 text-sm rounded-full text-gray-800 focus:outline-none focus:ring-4 focus:ring-white/60 shadow-xl"
              />
              <button className="absolute right-2 top-1.5 bg-blue-600 text-white px-4 py-1.5 rounded-full hover:bg-blue-700 transition-colors shadow-md">
                Search
              </button>
            </div>
          </div>
          ...
          <button 
            ...
            className="bg-white text-blue-600 px-5 py-2.5 text-base font-semibold rounded-full hover:bg-gray-100 transition-colors shadow-xl"
          >
            {data.hero.ctaText}
          </button>
      ...
      <section className="py-8 bg-white">
        <div className="w-full px-2 sm:px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="p-4 rounded-xl bg-white/60 backdrop-blur-sm shadow-sm hover:shadow-md transform hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 cursor-pointer">
              <div className="text-2xl font-bold text-blue-600 mb-1">{data.stats.doctors.toLocaleString()}+</div>
              <div className="text-xs text-gray-600">Partner Hospitals</div>
            </div>
            <div className="p-4 rounded-xl bg-white/60 backdrop-blur-sm shadow-sm hover:shadow-md transform hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 cursor-pointer">
              <div className="text-2xl font-bold text-green-600 mb-1">{data.stats.patients.toLocaleString()}+</div>
              <div className="text-xs text-gray-600">Patients Served</div>
            </div>
            <div className="p-4 rounded-xl bg-white/60 backdrop-blur-sm shadow-sm hover:shadow-md transform hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 cursor-pointer">
              <div className="text-2xl font-bold text-purple-600 mb-1">{data.stats.cities}+</div>
              <div className="text-xs text-gray-600">Cities Covered</div>
            </div>
            <div className="p-4 rounded-xl bg-white/60 backdrop-blur-sm shadow-sm hover:shadow-md transform hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 cursor-pointer">
              <div className="text-2xl font-bold text-yellow-600 mb-1">{data.stats.reviews.toLocaleString()}+</div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {data.categories.map((category, index) => (
              ...
              >
                <div
                  className="w-16 h-16 rounded-full flex items-center justify-center text-2xl mx-auto mb-4 group-hover:scale-110 transition-transform text-white"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {data.features.map((feature, index) => (
              <div key={index} className="text-center">
                <div
                  className="w-20 h-20 rounded-full flex items-center justify-center text-3xl mx-auto mb-6 text-white"
      ...
      {/* ============================================================================ */}
      <section className="py-16 text-white" style={{ background: 'linear-gradient(135deg, #00ff87 0%, #60efff 100%)' }}>
