...
export default function HomePage() {
  ...
  const fetchNearbyDoctors = async (lat: number, lon: number, radiusKm: number = 10) => {
    try {
      setMapLoading(true);
      setMapError(null);
      ...
      setMapPins(docs.map((d: any) => ({
        id: d.id,
        lat: Number(d.lat),
        lon: Number(d.lon),
        title: d?.profile?.clinicName || d.email?.split('@')[0] || `Doctor ${d.id}`,
        subtitle: `${d?.profile?.specialization || 'General'} • ${d.distanceKm} km`
      })));
    } catch (e: any) {
      setMapError(e?.message || 'Failed to load nearby doctors');
    } finally {
      setMapLoading(false);
    }
  };
  ...
  const detectLocation = async () => {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            ...
            fetchNearbyDoctors(latitude, longitude, 10).then(resolve).catch(reject);
          },
          (err) => reject(err),
      ...
      setMapError(e?.message || 'Unable to detect location');
    } finally {
      setMapLoading(false);
    }
  };
  ...
  const searchByCity = async (city: string) => {
    try {
      ...
      setMapError(e?.message || 'Search failed');
    } finally {
      setMapLoading(false);
    }
  };
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      ...
                      <label className="flex items-center justify-center px-3 py-2 text-sm bg-white border-2 border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-all">
                        ...
                        />
                        <span className="text-gray-900 font-medium">Availability Only</span>
                      </label>
                    </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
              {categories.map((category, index) => (
                ...
                >
                  <Link href={category.link} className="block h-full" prefetch={true}>
                    <div className="relative bg-white rounded-2xl p-4 md:p-6 text-center shadow-lg hover:shadow-2xl transition-all border border-gray-100 overflow-hidden h-full flex flex-col items-center justify-center">
                      {/* Gradient overlay on hover */}
                      <div className={`absolute inset-0 bg-gradient-to-br ${category.color} opacity-0 group-hover:opacity-10 transition-opacity`}></div>
                      
                      <div className={`relative w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br ${category.color} rounded-xl md:rounded-2xl flex items-center justify-center mx-auto mb-3 md:mb-4 group-hover:scale-110 transition-transform shadow-lg flex-shrink-0`}>
                        <category.icon className="w-6 h-6 md:w-8 md:h-8 text-white" />
                      </div>
                      <h3 className="text-sm md:text-base font-bold text-gray-900 mb-1 leading-tight">{category.title}</h3>
                      <p className="text-[10px] md:text-sm text-gray-600 leading-tight">{category.description}</p>
                    </div>
                  </Link>
                </motion.div>
              ))}
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-12">
              {stats.map((stat, index) => (
                ...
                >
                  <div className="bg-white rounded-2xl p-4 md:p-6 shadow-lg hover:shadow-2xl transition-all border border-gray-100 text-center overflow-hidden h-full flex flex-col justify-center">
                    {/* Gradient overlay on hover */}
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-purple-500/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    
                    <div className="relative">
                      <div className="w-10 h-10 md:w-14 md:h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl md:rounded-2xl flex items-center justify-center mx-auto mb-3 md:mb-4 shadow-lg">
                        <stat.icon className="w-5 h-5 md:w-7 md:h-7 text-white" />
                      </div>
                      <div className="text-xl sm:text-2xl md:text-4xl font-black text-gray-900 mb-1 md:mb-2 break-words leading-tight">
                        {stat.value.toLocaleString()}+
                      </div>
                      <div className="text-[10px] sm:text-xs md:text-sm font-bold text-gray-600 uppercase tracking-wider leading-tight">
                        {stat.label}
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              <motion.div
                ...
                className="bg-white rounded-2xl p-4 md:p-6 shadow-lg border border-gray-100 text-center flex flex-col items-center"
              >
                ...
                <h3 className="text-base md:text-lg font-bold text-gray-900 mb-1 md:mb-2 leading-tight">Verified Doctors Only</h3>
                <p className="text-xs md:text-sm text-gray-600 leading-relaxed">All doctors verified by license ID</p>
              </motion.div>
              ...
              >
                ...
                <h3 className="text-base md:text-lg font-bold text-gray-900 mb-1 md:mb-2 leading-tight">Real Reviews</h3>
                <p className="text-xs md:text-sm text-gray-600 leading-relaxed">10,000+ verified patient reviews</p>
              </motion.div>
              ...
              >
                ...
                <h3 className="text-base md:text-lg font-bold text-gray-900 mb-1 md:mb-2 leading-tight">Quality Assured</h3>
                <p className="text-xs md:text-sm text-gray-600 leading-relaxed">Premium healthcare standards</p>
              </motion.div>
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
              <div>
                <div className="bg-white rounded-xl p-8 border border-gray-200 shadow-md">
                  <h3 className="text-lg font-bold text-gray-900 mb-3">Doctors Near You</h3>
                  {mapError && <div className="text-sm text-red-600 mb-2">{mapError}</div>}
                  {mapCenter ? (
                    <MapDoctors center={mapCenter} pins={mapPins} height={320} />
                  ) : (
                    <div className="h-64 flex items-center justify-center text-gray-600">
                      {mapLoading ? 'Loading map…' : 'Use the options on the right to find doctors by location'}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-white rounded-xl p-5 shadow-md border border-gray-200">
                  ...
                  <button onClick={detectLocation} disabled={mapLoading} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold py-3 px-4 rounded-lg transition-all hover:from-blue-700 hover:to-indigo-700 shadow-md disabled:opacity-50">
                    {mapLoading ? 'Finding…' : 'Find Nearby Doctors'}
                  </button>
                </div>
                ...
                <div className="bg-white rounded-xl p-5 shadow-md border border-gray-200">
                  ...
                  <div className="space-y-3">
                    ...
                    />
                    <button onClick={() => searchByCity(selectedCity)} disabled={mapLoading} className="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white font-bold py-2 px-4 rounded-lg transition-all hover:from-green-700 hover:to-emerald-700 shadow-md disabled:opacity-50">
                      {mapLoading ? 'Searching…' : 'Show Doctors'}
                    </button>
                  </div>
                </div>
              </div>
