...
class ApiClient {
  ...
  // ============================================================================
  async getDoctors(params: { sort?: string; page?: number; pageSize?: number; time?: string; availabilityOnly?: boolean } = {}): Promise<Doctor[]> {
    ...
    if (params.availabilityOnly) queryParams.availabilityOnly = 'true';
  ...
  async searchDoctors(q: string, params: { time?: string; availabilityOnly?: boolean } = {}): Promise<SearchDoctorsResponse> {
  ...
  async getHospitals(params: { page?: number; limit?: number; city?: string; state?: string; q?: string } = {}): Promise<any[]> {
    const query = new URLSearchParams({
      page: String(params.page || 1),
  ...
  // Walk-in reservation (doctor)
  async reserveWalkInSlot(payload: { name: string; date: string; time: string; phone?: string }): Promise<{ appointment: any }> {
    return this.request('/api/doctor/walk-in/reserve', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  }
  ...
  // --- Doctor Token Queue ---
  async getDoctorTokensToday(doctorId: number): Promise<{ currentToken: number; tokens: Array<{ token: number; appointmentId: number; patientId: number; time: string }> }> {
