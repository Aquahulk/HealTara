...
import { getQueue, setQueue, setCurrentToken } from './tokenQueueStore';
...
function convert12hTo24h(time12h: string): string {
  ...
  if (parts.length < 2) return time12h; // already 24h
...
function isDoctorAvailable(doctor: any, timeSlot: string): boolean {
  if (!timeSlot) return true;
  const profile = doctor.doctorProfile;
  if (!profile || !profile.workingHours || profile.workingHours.length === 0) return true; // fallback: always available if no hours set

  const dayOfWeek = new Date().getDay(); // 0-6 (Sun-Sat)
  const reqTime = convert12hTo24h(timeSlot);

  const hours = profile.workingHours.find((h: any) => h.dayOfWeek === dayOfWeek);
  if (!hours) return false; // not working today

  return reqTime >= hours.startTime && reqTime <= hours.endTime;
}
...
app.get('/api/doctors', async (req: Request, res: Response) => {
  try {
    ...
    // First get candidate IDs (we'll filter them if needed)
    ...
    if (candidateIds.length === 0 && (availabilityOnly || timeSlot)) {
      return res.status(200).json({ success: true, data: [], count: 0 });
    }

    // Use raw query for efficient sorting and counting with ratings
    // Filter by candidateIds if availability filter was applied
    const whereClause = candidateIds.length > 0 
      ? `AND u.id IN (${candidateIds.join(',')})`
      : '';
...
app.post('/api/doctor/walk-in/reserve', authMiddleware, async (req: Request, res: Response) => {
  const user = req.user!;
  if (user.role !== 'DOCTOR') {
    return res.status(403).json({ message: 'Forbidden: Only doctors can reserve walk-in slots.' });
  }
  const { name, date, time, phone } = (req.body || {}) as { name?: string; date?: string; time?: string; phone?: string };
  if (!name || !date || !time) {
    return res.status(400).json({ message: 'Name, date (YYYY-MM-DD) and time (HH:mm) are required.' });
  }
  try {
    ...
    if (existingCount >= capacity) {
      return res.status(409).json({ message: 'Capacity reached for this hour.' });
    }
    let slot = await prisma.slot.findFirst({
      where: { doctorId: user.userId, date: { gte: dayStart, lte: dayEnd }, time: String(time) }
    });
    if (!slot) {
      slot = await prisma.slot.create({
        data: { doctorId: user.userId, date: new Date(date), time: String(time), status: 'BOOKED' }
      });
    } else {
    ...
    const email = `walkin.${user.userId}.${Date.now()}@local`;
    const walkin = await prisma.user.create({
      data: {
        ...
        canLogin: false,
        patientProfile: {
          create: { name: String(name), phone: phone ? String(phone) : undefined }
        }
      },
      select: { id: true }
    });
    const newAppointment = await prisma.appointment.create({
      data: {
        ...
        status: 'CONFIRMED',
        notes: 'WALK_IN',
        doctor: { connect: { id: user.userId } },
        patient: { connect: { id: walkin.id } },
      },
    });
    try {
      broadcastDoctorEvent(user.userId, 'appointment-booked', { doctorId: user.userId, appointmentId: newAppointment.id });
      broadcastDoctorEvent(user.userId, 'slots:updated', { doctorId: user.userId, date, time });
      const memberships = await prisma.hospitalDoctor.findMany({ where: { doctorId: user.userId }, select: { hospitalId: true } });
      for (const m of memberships) {
        broadcastHospitalEvent(m.hospitalId, 'appointment-booked', { doctorId: user.userId, appointmentId: newAppointment.id });
        broadcastHospitalEvent(m.hospitalId, 'slots:updated', { doctorId: user.userId, date, time });
      }
    } catch {}
    return res.status(201).json({ message: 'Walk-in slot reserved', appointment: newAppointment });
  } catch (error) {
    console.error('walk-in reserve error', error);
    return res.status(500).json({ message: 'Failed to reserve walk-in slot' });
  }
});
// =================================================================
// --- ADMIN ENDPOINTS ---
...
// --- Geocoding proxy using Nominatim (OSM) with simple file cache ---
app.get('/api/geo/geocode', async (req: Request, res: Response) => {
  const query = String(req.query.query || '').trim();
  if (!query) return res.status(400).json({ message: 'query required' });
  try {
    const cached = getGeocache(query);
    if (cached) return res.status(200).json({ lat: cached.lat, lon: cached.lon, cached: true });
    const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}`;
    const resp = await fetch(url, { headers: { 'User-Agent': 'healtara/1.0' } });
    const arr = await resp.json();
    const first = Array.isArray(arr) && arr.length ? arr[0] : null;
    if (!first) return res.status(404).json({ message: 'not found' });
    const lat = Number(first.lat), lon = Number(first.lon);
    if (Number.isFinite(lat) && Number.isFinite(lon)) setGeocache(query, lat, lon);
    return res.status(200).json({ lat, lon, cached: false });
  } catch (error) {
...
app.get('/api/doctors/near', async (req: Request, res: Response) => {
  ...
  try {
    const doctors = await prisma.user.findMany({
      ...
      select: {
        id: true, email: true,
        doctorProfile: { select: { clinicName: true, clinicAddress: true, city: true, state: true, phone: true, specialization: true } }
      }
    });
    ...
    for (const d of doctors) {
      ...
      if (!dp) continue;
      const q = [dp.clinicAddress, dp.city, dp.state].filter(Boolean).join(', ');
      if (!q) continue;
      let entry = getGeocache(q);
      if (!entry) {
        try {
          ...
          const first = Array.isArray(arr) && arr.length ? arr[0] : null;
          if (first) {
            const glat = Number(first.lat), glon = Number(first.lon);
            if (Number.isFinite(glat) && Number.isFinite(glon)) {
              setGeocache(q, glat, glon);
              entry = { query: q, lat: glat, lon: glon, ts: Date.now() } as any;
            }
          }
        } catch {}
      }
    ...
    return res.status(200).json({ doctors: enriched });
  } catch (error) {
    return res.status(500).json({ message: 'Failed to search nearby doctors' });
  }
});

// --- Doctor Daily Token Queue ---
app.get('/api/doctors/:doctorId/tokens/today', async (req: Request, res: Response) => {
  try {
    const doctorId = Number(req.params.doctorId);
    if (!Number.isFinite(doctorId)) return res.status(400).json({ message: 'Invalid doctorId' });
    ...
    const tokens = appts.map(
      (a: { id: number; patientId: number; time: string }, idx: number) => ({
        token: idx + 1,
        appointmentId: a.id,
...
app.post('/api/doctors/:doctorId/tokens/next', authMiddleware, async (req: Request, res: Response) => {
  try {
    const doctorId = Number(req.params.doctorId);
    if (!Number.isFinite(doctorId)) return res.status(400).json({ message: 'Invalid doctorId' });
    const user = req.user!;
    ...
    const existing = getQueue(doctorId, todayStr);
    const current = existing?.currentToken ?? 0;
    const next = tokens.find((t: { token: number }) => t.token > current);
    const newToken = next ? next.token : current;
    ...
    try {
      ...
      const patientIds = Array.from(
        new Set(tokens.map((t: { patientId: number }) => t.patientId))
      ...
      for (const m of memberships) {
        broadcastHospitalEvent(m.hospitalId, 'token:updated', { doctorId, date: todayStr, currentToken: newToken });
      }
    } catch {}
    return res.status(200).json({ currentToken: newToken, tokens });
  } catch (error) {
    console.error('tokens next error', error);
    return res.status(500).json({ message: 'Failed to advance token' });
  }
});
...
app.get('/api/hospitals', async (req: Request, res: Response) => {
  try {
    ...
    if (city) {
      filtered = filtered.filter((h: any) => String(h.city || '').toLowerCase().includes(city));
    }
    if (state) {
      filtered = filtered.filter((h: any) => String(h.state || '').toLowerCase().includes(state));
    }
    if (q) {
      filtered = filtered.filter((h: any) => {
        const name = String(h.name || '').toLowerCase();
        const c = String(h.city || '').toLowerCase();
        const s = String(h.state || '').toLowerCase();
        return name.includes(q) || c.includes(q) || s.includes(q);
      });
    }
    // Apply pagination to filtered cached data
    const paginatedHospitals = filtered.slice(skip, skip + limit);
